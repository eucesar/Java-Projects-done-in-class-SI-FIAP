...
public class ConnectionFactory {
	...
	// Construtor privado para impedir a criação de múltiplas instâncias - conectar ao servidor
	private ConnectionFactory() {
		...
		try (FileInputStream file = new FileInputStream("config.properties")) {
			...
			USERNAME = properties.getProperty("USERNAME"); //ter q estar igualzinho para funcionar no banco de dados - no file - esconder senha e login e loga no file so para isso
	...
	// Método que retorna a única instância da ConnectionFactory - conecxão com o servidor, se n tiver ngm usando volta instance
	// conecxão geral
	public static ConnectionFactory getInstance() {
		if (instance == null) {
			synchronized (ConnectionFactory.class) {
				//se já tiver outra pessoa conectada ele abre uma nova instancia e conecta
				if (instance == null) {
