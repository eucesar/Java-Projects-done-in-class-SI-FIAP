...
public class CadastrousuarioServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// obtem o valor do salt
		byte[] salt = Util.getSalt();
		
		// aplicar o algoritmo de hash
		String senhaCodificada = Util.codificar(request.getParameter("senha"), salt);
		
		//configuração dos atributos do objeto usuario
		perfil.setPerfil(request.getParameter("perfil")); //request - chamar
		...
		response.sendRedirect("index.html"); //vai enviar para o index
	}
